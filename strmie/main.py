...
def main():
    ...
    from strmie.scripts.cnn import *
...
<<<<<<< Updated upstream
        if cnn:
            ...
            coppie_PE(file_names)
            data.drop(["Seq","ID"],axis=1,inplace=True)

            hist_data = pd.crosstab(data['filename'], data['CAG_repeats']).reset_index()


            print("Create dataframe for CNN")


            # Ottimizzazione delle operazioni su DataFrame
            hist_data['reads'] = hist_data['filename'].str.split("_L001_").str.get(1).str.split(".fastq.gz").str.get(0)
            hist_data['filename'] = hist_data['filename'].str.split("_L001_R").str.get(0)
            hist_data.set_index(["filename", "reads"], inplace=True)
            #print(hist_data)
            print("Running CNN")
            peaks_predicted = cnn_find_peaks(hist_data)
            peaks_predicted.to_excel(path + "predicted_peaks_.xlsx")

            out = "metrics_and_predicted_peaks_.xlsx"
            metrics_for_predicted_peaks(data, campioni, path + out, peaks_predicted)
